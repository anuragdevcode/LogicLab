...
import { drawSortBars, clear } from './CanvasRenderer';
...
export const SortingCanvas: React.FC = () => {
  ...
  const step = module === 'sorting' && stepIdx > 0 && stepIdx <= steps.length
    ? (steps[stepIdx - 1] as SortStep)
    : arr && arr.length > 0
    ? { arr, cmp: [], done: [], swap: false, line: -1 }
    : null;
  ...
  useEffect(() => {
    ...
    if (step) {
      drawSortBars(ctx, width, height, step);
    } else {
